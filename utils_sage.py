...
def load_data(prefix, normalize=True, load_walks=True, root_dir='data'):
    G_data = json.load(open("{}/{}-G.json".format(root_dir, prefix)))
    ...
    if os.path.exists("{}/{}-feats.npy".format(root_dir, prefix)):
        feats = np.load("{}/{}-feats.npy".format(root_dir, prefix))
    else:
        print("No features present.. Only identity features will be used.")
        feats = None
    id_map = json.load(open("{}/{}-id_map.json".format(root_dir, prefix)))
    id_map = {conversion(k): int(v) for k, v in id_map.items()}
    walks = []
    class_map = json.load(
        open("{}/{}-class_map.json".format(root_dir, prefix)))
    ...
    if load_walks:
        with open("{}/{}-walks.txt".format(root_dir, prefix)) as fp:
            for line in fp:
...
if __name__ == "__main__":
    ...
    # out_file = sys.argv[2]
    graph_file = 'data/pubmed-G.json'
    out_file = 'data/pubmed-walks.txt'
